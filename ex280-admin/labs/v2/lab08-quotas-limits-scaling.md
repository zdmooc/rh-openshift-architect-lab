...
- `LimitRange` `lr-small` appliqué.
- Deployment `quota-web` avec plusieurs replicas après scale.
- HPA `quota-web` présent avec min/max + cible CPU.
...
- Pods en `Pending` après scale :
  ...
  - `oc describe resourcequota rq-small`
- HPA non créé :
  - Vérifier que le composant de metrics est disponible.
  - Vérifier la syntaxe `oc autoscale`.
